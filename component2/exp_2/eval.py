        ...
        for alpha in alphas:
            for prefactor in prefactors:
                ...
                for key in sorted(keys_present, key=lambda x: x[3]):
                    ...
                    if len(r["train_losses"]) == 0 or len(r["test_losses"]) == 0:
                        continue

                    train_curves.append(r["train_losses"])
                    test_curves.append(r["test_losses"])

                if len(train_curves) == 0 or len(test_curves) == 0:
                    continue

                min_len = min(len(curve) for curve in train_curves)
                min_len = min(min_len, min(len(curve) for curve in test_curves))

                train_curves = [curve[:min_len] for curve in train_curves]
                test_curves = [curve[:min_len] for curve in test_curves]
    ...
    for regime in regimes:
        for alpha in alphas:
            for prefactor in prefactors:
                ...
                if len(train_learn_score_across_widths) == 0 or len(test_learn_score_across_widths) == 0:
                    continue
